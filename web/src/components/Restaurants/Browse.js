  ...
  return (
    <div>
      <section className="about-us">
        <Container>
          <div className="about">
            <h1 style={{ fontWeight: "bold" }}>Special Offers !</h1>
      ...
      <Form style={{ width: "50%", margin: "auto", marginTop: "10rem" }}>
      ...
      >
        {Object.keys(restaurants)
          .filter((key) =>
            restaurants[key].title.toLowerCase().includes(filterText)
          )
          .map((restKey) => (
            // <VisibilitySensor
            //   onChange={(isVisible) => onChange({ isVisible, restKey })}
            // >
            <Link className="noHover" to={`/menu/${restaurants[restKey]._id}`}>
              <Card style={{ width: "18rem", margin: "2rem 0" }}>
                <Card.Header style={{ display: "flex" }}>
                  <Distance
                    lat1={user.lat}
                    lon1={user.lng}
                    lat2={restaurants[restKey].coordinatelatitude}
                    lon2={restaurants[restKey].coordinatelongitude}
                  />
                </Card.Header>
                <Card.Img
                  variant="top"
                  src={restaurants[restKey].image}
                  height="220px"
                />
                <Card.Body>
                  <Card.Title
                  ...
                  >
                    {restaurants[restKey].title}
                  </Card.Title>
                  <Card.Text style={{ color: "black", textDecoration: "none" }}>
                    {restaurants[restKey].description.slice(0, 50) + "..."}
                  </Card.Text>
                  <ReactStars
                    count={5}
                    value={restaurants[restKey].rating}
                    size={24}
                    activeColor="#ffd700"
                    edit={false}
                  />
